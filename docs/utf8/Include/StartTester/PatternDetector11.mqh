...
// -------------------------------------------------------------------
// Opis:  The same as DrawTradeArrowByIndex, ale po czasie świecy.
...
//        (stops/freeze), a następnie wywołuje ExecuteImpulseTrade z parametrami **per regime**.
...
//           SymbolInfoInteger/SymbolInfoDouble, GetRegimeExitConfig, ExecuteImpulseTrade (overload).
// Używa globalnych: candleHistory[], inputOneSignalPerBarPerDirection, inputExecuteMarginPoints,
//                   inputMagicNumber, _Point, _Symbol, impulseCandlesSinceLastDetection.
// -------------------------------------------------------------------
void DetectLivePatternAndDraw()
{
   ...
   // === EXIT per regime: pobierz metodę i parametry z inputów CSV i użyj overloadu ===
   int useM; double slM, slP, tpM;
   GetRegimeExitConfig(reg, useM, slM, slP, tpM);

   ExecuteImpulseTrade(isBuy, adj, t1, useM, inputMagicNumber, slM, slP, tpM);
...
//        i rysuje strzałkę tylko dla zyskownych przypadków. Używa parametrów **per regime**.
// Wywołuje: CheckImpulseConditions, GetRegimeExitConfig, CalculateSLAndTP, SymbolInfoDouble,
//           SimulatePendingAndTradePoints, DrawTradeArrow, PD_DrawUnifiedRegimeMA.
// Używa globalnych: candleHistory[], inputExecuteMarginPoints, inputPendingExpiryBars, _Symbol.
// -------------------------------------------------------------------
void DetectHistoricalImpulsesWithProfitCheck()
{
   ...
   for (int i = ArraySize(candleHistory) - 2; i >= 10; --i)
   {
      ...
      CalculateSLAndTP(sl, tp, useM, adjusted, isBuy, slM, slP, tpM);

      if (sl == 0.0 || tp == 0.0)
      {
         double range = candleHistory[i].high - candleHistory[i].low;
         double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
         double base  = MathMax(slP * point, slM * range);
         if (base <= 0) base = 10 * point;

         if (isBuy) { sl = adjusted - base; tp = adjusted + base * tpM; }
         else       { sl = adjusted + base; tp = adjusted - base * tpM; }
      }
