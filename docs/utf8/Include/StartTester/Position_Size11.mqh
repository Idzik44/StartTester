...
// ─────────────────────────────────────────────────────────────
// Helper: _MinSafeDistancePoints
// Opis:  Wyznacza minimalny dozwolony dystans cenowy na podstawie
//        max(SYMBOL_TRADE_STOPS_LEVEL, SYMBOL_TRADE_FREEZE_LEVEL) * _Point.
// Używa: SymbolInfoInteger(), SymbolInfoDouble(), _Symbol.
// Zwraca: dystans w jednostkach ceny (double).
// ─────────────────────────────────────────────────────────────
double _MinSafeDistancePoints()
{
    double point   = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    int stopsP     = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
    int freezeP    = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_FREEZE_LEVEL);
    int reqP       = (stopsP > freezeP ? stopsP : freezeP);
    return reqP * point;
}

// ─────────────────────────────────────────────────────────────
// Helper: _RoundToTick
// Opis:  Zaokrągla cenę do najbliższego ticka i normalizuje do _Digits.
// Używa: SymbolInfoDouble(), _Symbol, _Digits, MathRound().
// Zwraca: cena po „snapie” do siatki ticków.
// ─────────────────────────────────────────────────────────────
double _RoundToTick(double price)
{
    double ts = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
    if (ts <= 0.0) return NormalizeDouble(price, _Digits);
    double rounded = MathRound(price / ts) * ts;
...
//          [Poprawka]: Priorytet kapitału: najpierw argument AccountRiskCapital (>0),
//          potem inputAccountRiskCapital, na końcu ACCOUNT_MARGIN_FREE.
//          [Poprawka]: Zwracany lot zaokrąglany do liczby miejsc wynikającej z VOLUME_STEP.
...
// Zwraca:  lotSize znormalizowany do precyzji wynikającej z kroku wolumenu.
// ─────────────────────────────────────────────────────────────
    ...
    double RiskPercentage)
{
    ...
    // Priorytet kapitału ryzyka: argument > input > wolny margin
    double riskCapitalToUse = (AccountRiskCapital > 0.0)
                              ? AccountRiskCapital
                              : ((inputAccountRiskCapital > 0.0)
    ...
    else if (CalculationMode == 1) // Procent ryzyka
    {
        ...
        if (stopLossDistancePoints <= 0.0)
        {
            ...
            if (stopLossDistancePoints <= 5) // Minimalny SL (w tickach)
                return 0.0;
        }
    ...
    // Precyzja liczby miejsc po przecinku wg volume step (np. 0.01 -> 2, 0.001 -> 3)
    int volDigits = 2;
    if (volStep > 0.0)
    {
        int digits = 0;
        double step = volStep;
        // policz ile miejsc po przecinku potrzeba, max do 8
...
//          (1) konsolidacja: entry ± range*SLMultiplier, gdzie
//              range = (max(high) - min(low)) z ostatnich inputSL_RangeBars świec,
//          (2) stałe SLPoints,
...
//          [Poprawka]: Weryfikacja minimalnego dystansu (Stops/Freeze) dla metod 1 i 2,
//          oraz „snap” SL/TP do ticka na końcu.
// Woła:    SymbolInfoDouble(), SymbolInfoInteger(), NormalizeDouble(), Print().
// Używa:   _Symbol, _Digits, _Period; Highs[], Lows[] (PeaksEnded11.mqh) – dla metody 0;
//          candleHistory[] – dla metod 1 i 3; inputSL_RangeBars – dla metody 1.
...
void CalculateSLAndTP(
    ...
    double TPMultiplier)
{
    // --- Lokalne wyznaczenie zasięgu konsolidacji (zamiast globalnych minLow/maxHigh) ---
    double localMinLow  = 0.0;
    double localMaxHigh = 0.0;
    double range        = 0.0;

    // Liczymy na podstawie ostatnich zamkniętych świec: [1..K]
    int total = ArraySize(candleHistory);
    int K = (total > 1 ? MathMin(inputSL_RangeBars, total - 1) : 0);

    if (K > 0)
    {
        localMinLow  = candleHistory[1].low;
        localMaxHigh = candleHistory[1].high;

        for (int i = 1; i <= K; ++i)
        {
            if (candleHistory[i].low  < localMinLow)  localMinLow  = candleHistory[i].low;
            if (candleHistory[i].high > localMaxHigh) localMaxHigh = candleHistory[i].high;
        }
        range = localMaxHigh - localMinLow;
    }
    else
    {
        range = 0.0;
    ...
    // ----------------------------------------------------------------
    if (range <= 0 && UseSLMethod == 1) // tylko metoda konsolidacji wymaga 'range'
    {
        stopLossPrice   = 0;
        takeProfitPrice = 0;
        return;
    ...
    double point           = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double minStopDistance = _MinSafeDistancePoints();

    if (UseSLMethod == 0) // Metoda szczytów/dołków (Highs/Lows)
    {
        stopLossPrice = 0;
    ...
    else if (UseSLMethod == 1) // Metoda konsolidacji (entry ± range * SLMultiplier)
    {
        ...
        // Minimalny dystans (Stops/FREEZE)
        if (stopLossPrice > 0 && MathAbs(entryPrice - stopLossPrice) < minStopDistance)
        {
            stopLossPrice = isBuy
    ...
    else if (UseSLMethod == 2) // SL w punktach
    {
        if (SLPoints > 0)
        {
            ...
            // Minimalny dystans (Stops/FREEZE)
            if (MathAbs(entryPrice - stopLossPrice) < minStopDistance)
            {
                stopLossPrice = isBuy
                                ? entryPrice - minStopDistance
                                : entryPrice + minStopDistance;
    ...
    // Snap do ticka (ostateczne „bezpieczne” zaokrąglenie)
    if (stopLossPrice   > 0) stopLossPrice   = _RoundToTick(stopLossPrice);
    if (takeProfitPrice > 0) takeProfitPrice = _RoundToTick(takeProfitPrice);
}
